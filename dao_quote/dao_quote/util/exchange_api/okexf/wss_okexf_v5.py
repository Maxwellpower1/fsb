...
from dao_quote.util.exchange_api.okexf import api_okf_v5
    ...
    while True:
        try:
            ...
            if (time.time() > expire_time):
                symbol_dict, r_symbol_dict = api_okf_v5.convert_wss_symbol()
...
def okexf_on_message(ws, message):
    ...
    symbol = process_okexf_r_symbol(data['arg']['instId'])
    if (table == 'trades'):
        type = 'trade'
...
def send_event_dict(symbol, type, data):
    ...
    if type == 'kline':
        bar_pre = data[0]
        bar = []
        bar = [int(bar_pre[0]), float(bar_pre[1]), float(bar_pre[2]), float(bar_pre[3]), float(bar_pre[4]), float(bar_pre[5]), float(bar_pre[6])]
        timestamp = int(bar[0])
        close = float(bar[4])
